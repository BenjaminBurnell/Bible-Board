 ...
 *
 * 1.  Pre-computation: Book names and aliases are normalized *once*
 * at module load (`precomputedBibleBooks`) instead of on every
 * call to `findBibleVerseReference`. This significantly speeds
 * up the fuzzy matching logic.
 * 2.  Web Worker: **Skipped.** Moving the synchronous `stringSimilarity`
 * function to an async worker would break the contract of
 * `findBibleVerseReference`, which `script.js` relies on to be
 * synchronous. The pre-computation (step 1) provides the
 * majority of the requested performance gain by removing
 * per-keystroke normalization work.
 * ================================================================
 */
...
/**
 * OPTIMIZATION: Pre-compute normalized book names and aliases
 * at module load time.
 */
const precomputedBibleBooks = bibleBooks.map(([book, aliases]) => ({
  book: book,
  // Normalize all variants (book name + aliases) once
  variants: [book, ...aliases].map(norm),
}));
...
function findBibleVerseReference(input) {
  ...
  // OPTIMIZATION: Iterate precomputed list
  for (const { book, variants } of precomputedBibleBooks) {
    for (const vNorm of variants) {
  ...
  if (exactBook) {
    ...
    // defaults
    if (!finalChapter && !finalVerse) {
      finalChapter = 1;
      finalVerse = 1;
    } else if (finalChapter && !finalVerse) {
      finalVerse = 1;
    }
